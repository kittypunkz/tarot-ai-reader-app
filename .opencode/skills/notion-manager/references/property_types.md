...
### Updating Status (for work tracking)
```python
properties = {
...
```bash
python .opencode/skills/notion-manager/scripts/update_page.py PAGE_ID --properties '{"Status": {"status": {"name": "In Progress"}}}'
```
...
### Querying by Status
```bash
python .opencode/skills/notion-manager/scripts/query_database.py DATABASE_ID --filter '{"property": "Status", "status": {"equals": "In Progress"}}'
```
