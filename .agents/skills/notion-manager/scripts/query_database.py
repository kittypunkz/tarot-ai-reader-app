        ...
        for page in response.get("results", []):
            ...
            for prop_name, prop_data in page.get("properties", {}).items():
                ...
                if prop_type == "title":
                    ...
                    simplified["properties"][prop_name] = "".join([t.get("plain_text", "") for t in titles])
                elif prop_type == "rich_text":
                    texts = prop_data.get("rich_text", [])
                    simplified["properties"][prop_name] = "".join([t.get("plain_text", "") for t in texts])
                elif prop_type == "select":
                    select = prop_data.get("select")
                    simplified["properties"][prop_name] = select.get("name") if select else None
                    ...
                    simplified["properties"][prop_name] = status.get("name") if status else None
                elif prop_type == "checkbox":
                    simplified["properties"][prop_name] = prop_data.get("checkbox", False)
                    ...
                    simplified["properties"][prop_name] = [r.get("id") for r in relations]
                else:
...
def main():
    ...
    parser.add_argument("--page-size", "-n", type=int, default=100, help="Max results to return")
