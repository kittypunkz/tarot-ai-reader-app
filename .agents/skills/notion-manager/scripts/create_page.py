...
def main():
    ...
    parser.add_argument("--properties", "-props", help="JSON properties object for database pages")
    parser.add_argument("--content", "-c", help="JSON array of content blocks")
    
    args = parser.parse_args()
    
    if not args.database_id and not args.parent_id:
        print("Error: Must specify either --database-id or --parent-id", file=sys.stderr)
        sys.exit(1)
    ...
    if args.database_id:
        if not args.properties:
            print("Error: --properties required when creating database page", file=sys.stderr)
            sys.exit(1)
        ...
        if not args.title:
            print("Error: --title required when creating standalone page", file=sys.stderr)
            sys.exit(1)
